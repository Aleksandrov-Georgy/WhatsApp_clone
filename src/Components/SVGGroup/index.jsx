...
import { setChatId } from '../../Store/Slice/setUserMessageSlice';
...
const SVGGroup = () => {
  ...
  const [errorFormatNumber, setErrorFormatNumber] = React.useState(false);
  ...
  const pushMessage = () => {
    if (number.length === 11) {
      setErrorFormatNumber(false);
      dispatch(setChatId(number));
      setShowPopup(!showPopup);
    } else {
      setErrorFormatNumber(true);
    }
  };
  ...
  return (
    <div ref={popupRef} className={styles.content_left_svgGroup}>
      {showPopup && (
        <div className={styles.numberBox}>
          <h3>Введите номер собеседника</h3>
          <div className={styles.numberBox_input}>
            <input
              value={number}
            ...
            />
            {number && <AiOutlineCloseCircle className={styles.svg} onClick={() => clearInput()} />}
          </div>
          <button onClick={() => pushMessage()}>Написать</button>
          {errorFormatNumber && (
            <div className={styles.error_format}>
              <h6>Не верный формат номера, укажите телефон в формате 79991234567</h6>
            </div>
          )}
        </div>
      )}
      ...
      <svg
        ...
        className={styles.content_left_svgColor}
        fill="none">
        <path
