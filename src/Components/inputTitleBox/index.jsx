...
import { getOutGoingMessage } from '../../Store/Slice/listMessageSlice';

const InputTitleBox = () => {
  const dispatch = useDispatch();
  const userID = useSelector((state) => state.login.idInstance);
  const userAPI = useSelector((state) => state.login.apiTokenInstance);
  const chatNumber = useSelector((state) => state.pushMessage.chatId);
  ...
  const config = {
    ...
    url: `https://api.green-api.com/waInstance${userID}/sendMessage/${userAPI}`,
  ...
  const pushMessage = () => {
    axios
      .request(config)
      .then((response) => {
        dispatch(getOutGoingMessage(message));
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  return (
    <div className={styles.content_right_inputBox}>
      <svg
        viewBox="0 0 24 24"
        height="24"
        width="24"
